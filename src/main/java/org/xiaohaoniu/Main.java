...
package org.xiaohaoniu;


import org.xiaohaoniu.CommonMethod.osSelect;
import org.xiaohaoniu.NetSDKDemo.HCNetSDK;
import com.sun.jna.Native;
import com.sun.jna.Pointer;
import org.xiaohaoniu.alarm.FMSGCallBack;
import org.xiaohaoniu.alarm.FMSGCallBack_V31;
...
//                        strDllPath = "E:\\haikangdemo\\lib\\HCNetSDK.dll";
    ...
    public static void main(String[] args) throws UnknownHostException {
        ...
        lUserID= Main.loginDevice( "192.168.0.64", (short) 8000, "admin", "qazwsx12");  //登录设备
        ...
        for (boolean exit = false; !exit; ) {
            ...
            switch (str) {
                ...
                {
                    System.out.println("\n[Module]报警布防示例代码");
                    lAlarmHandle= Main.setAlarmChan(lUserID);//报警布防，和报警监听二选一即可 实测可用
                    break;
                }
                case "2":
                {
                    System.out.println("\n[Module]报警撤防示例代码");
                    Main.closedAlarmChan(lAlarmHandle);
                    break;
                }
                case "3":
                {
                    ...
                    System.out.println("\n[Module]开启报警监听示例代码");
                    lListenHandle = Main.startListen(ipAddress,(short) 7200); //传入监听PC本机的IP地址和端口
                    break;
                }
                case "4":
                {
                    System.out.println("\n[Module]停止监听示例代码");
                    Main.stopListen(lListenHandle);
                    break;
                }
                case "5":
                {
                    System.out.println("\n[Module]AI开放平台接入视频报警");
                    boolean b = hCNetSDK.NET_DVR_SetDVRMessageCallBack_V50(0, fMSFCallBack_V31, null);
                    if (b) {
                        System.out.println("\n注册成功");
                    }else{
                        System.out.println("\n注册失败");
